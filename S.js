function load_section14(mainContainer) {
  mainContainer.innerHTML = `
    <section id="report-section14">
      ...
      <div class="chart-container" style="position: relative; width: 100%; max-width: 600px; margin: 0 auto 30px; background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); height: 450px;">
  ...
  // Render sustainability radar chart – BIG SIZE ON ALL DEVICES
  ...
  if (canvas) {
    ...
    new Chart(ctx, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false,   // fills the container's height
        plugins: {
          title: {
            display: true,
            text: 'Sustainability Comparison — Material‑Level View',
            font: { size: window.innerWidth < 600 ? 12 : 16, weight: '600' },
            color: '#1a1a1a',
            padding: 15
          },
          legend: {
  ...
  // Ensure responsive container styles (LARGE chart)
