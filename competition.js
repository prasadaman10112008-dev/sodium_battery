function load_competition(mainContainer) {
  mainContainer.innerHTML = `
    <section id="competition-section">
      ...
      <div class="comp-chart-wrapper">
        <div class="comp-chart-inner">
          <canvas id="competitionMetricsChart"></canvas>
        </div>
      ...
      <!-- Differentiation Table (Why We Win in Specific Segments) – now scrollable on mobile -->
      <h3 style="margin-top: 40px;">Where We Win vs. Competitors</h3>
      <div style="overflow-x: auto; -webkit-overflow-scrolling: touch; margin-bottom: 20px;">
        <table style="min-width: 750px; width: 100%; border-collapse: collapse; background: var(--surface); font-size: 0.82rem;">
          <thead>
            <tr>
  ...
  // Real data bar chart – now with fixed container height so always visible
  ...
  // Inject styles once – with fixed chart container height
  if (!document.getElementById('comp-style')) {
    ...
    style.textContent = `
      ...
      /* Chart wrapper and inner container for fixed height */
      .comp-chart-wrapper {
        max-width: 750px;
        margin: 0 auto 36px;
        background: var(--surface);
      ...
      .comp-chart-inner {
        position: relative;
        width: 100%;
        height: 400px;               /* fixed height so chart always shows */
        overflow-x: auto;
        -webkit-overflow-scrolling: touch;
      }
      .comp-chart-inner canvas {
        display: block;
        width: 100% !important;
      ...
      @media (max-width: 600px) {
        .comp-chart-inner {
          height: 350px;             /* slightly smaller but still visible */
        }
        .comp-chart-inner canvas {
          min-height: 300px;
        }
